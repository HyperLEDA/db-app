...
CREATE TYPE morphology.MethodType AS ENUM ( 'expert' , 'citizen science', 'machine learning', 'parametric', 'nonparametric', 'kinematic', 'simplified' ) ;
...
CREATE TYPE morphology.ClassType AS ENUM ('elliptical', 'lenticular', 'spiral', 'irregular', 'spheroidal') ;
COMMENT ON TYPE morphology.ClassType IS '{
...
CREATE TYPE morphology.ExtraType AS ENUM ('UCD', 'cE', 'dE', 'dS0', 'dSph', 'UDG', 'UFD', 'Tr', 'Ir', 'Im', 'BCD','LSB','XLSB') ;
COMMENT ON TYPE morphology.ExtraType IS '{
"description": "Additional morphological and phenomenological galaxy types",
"values": {
...
-------------- Hubble sequence --------------
CREATE TABLE IF NOT EXISTS morphology.HubbleSequence (
  t	SmallInt	PRIMARY KEY
, class	morphology.ClassType	NOT NULL
, stage	Text	NOT NULL
, design	Text	NOT NULL
, description	Text	NOT NULL
) ;

COMMENT ON TABLE  morphology.HubbleSequence	IS 'The Hubble sequence' ;
COMMENT ON COLUMN morphology.HubbleSequence.t	IS '{"description": "de Vaucouleurs numeric type", "ucd": "src.morph.type"}' ;
COMMENT ON COLUMN morphology.HubbleSequence.class	IS '{"description": "Galaxy class: elliptical, lenticular, spiral, irregular, or spheroidal", "ucd": "meta.code.class"}' ;
COMMENT ON COLUMN morphology.HubbleSequence.stage	IS '{"description": "Intermediate stage within each class", "ucd": "meta.code.class;obs.sequence"}' ;
COMMENT ON COLUMN morphology.HubbleSequence.design	IS '{"description": "Hubble morphological type", "ucd": "src.morph.type"}' ;
COMMENT ON COLUMN morphology.HubbleSequence.description	IS '{"description": "Description", "ucd": "meta.note"}' ;

INSERT INTO morphology.HubbleSequence (t, class, stage, design, description) VALUES 
...
CREATE TYPE morphology.AttributeGroupType AS ENUM ( 'appearance' , 'environment' , 'bulge' , 'spiral pattern' , 'texture' , 'dynamics', 'interaction', 'peculiar') ;
COMMENT ON TYPE morphology.AttributeGroupType IS '{
...
CREATE TABLE IF NOT EXISTS morphology.attributes (
  id	Text	PRIMARY KEY
, attribute_group	morphology.AttributeGroupType	NOT NULL
, description	JSON
) ;

COMMENT ON TABLE morphology.attributes	IS 'Dictionary of attributes' ;
COMMENT ON COLUMN morphology.attributes.id	IS '{"description": "Attribute ID", "ucd": "meta.id"}' ;
COMMENT ON COLUMN morphology.attributes.attribute_group	IS 'Attribute group' ;
COMMENT ON COLUMN morphology.attributes.description	IS '{"description": "Attribute description", "ucd": "meta.note"}' ;
...
, ( 'b/t' , 'bulge',
'{
"description": "Bulge/Total ratio: relative contribution of the bulge to the total flux of the galaxy", 
"values": {
  "0": "no bulge", 
...
"values": {
  ...
  "0.25": "open spiral arms, with pitch angles of 30° to 40°", 
  "0.5": "moderately open spiral arms, with pitch angles of 20° to 30°", 
  "0.75": "closed-in spiral arms, with pitch angles of 10° to 20°", 
...
"values": {
  ...
  "0.75": "long bar, that extends over about half of D25", 
...
'{
"description": "Presence and relative prominence of an outer pseudo-ring (Buta & Combes, 1996): R1 pseudo-ring having a dimpled eight shape due to a 180◦ winding of the spiral arms relative to the bar; R2 pseudo-ring with a higher winding of 270◦; and an intermediate R1-R2 pattern", 
"values": {
...
'{
"description": "Amplitude of distortions of the galaxy profile", 
"values": {
  "0": "no distortion", 
...
-- , ( 'x-shape' , 'bulge', '{"description": "An X-shaped or peanut-shaped morphology of the central bulge, usually associated with a vertically thickened bar"}'::JSON )

-- , ( 'tidal tail' , 'interaction', '{"description": "A long, narrow, low-surface-brightness stellar structure extending from a galaxy, produced by tidal interaction or merger and generally connected to the parent galaxy"}'::JSON )
-- , ( 'tidal stream' , 'interaction', '{"description": "An elongated, diffuse, low-surface-brightness stellar structure produced by tidal stripping and tracing the orbit or disrupted material of a galaxy or satellite"}'::JSON )
-- , ( 'tidal bridge' , 'interaction', '{"description": "An elongated stellar or gaseous structure connecting two interacting galaxies or their tidal debris, produced by their mutual gravitational interaction"}'::JSON )
-- , ( 'tidal debris' , 'interaction', '{"description": "Diffuse stellar material distributed outside the main body of a galaxy as a result of tidal interaction, stripping or merger"}'::JSON )
-- , ( 'shell' , 'interaction', '{"description": "Diffuse, arc-like or shell-like stellar structures surrounding a galaxy, typically interpreted as phase-wrapped tidal debris from a past merger or accretion event"}'::JSON )
-- , ( 'fan' , 'interaction', '{"description": "A broad, diffuse, fan-shaped stellar structure extending from a galaxy, generally interpreted as tidal debris associated with an interaction, merger or accretion event"}'::JSON )
-- , ( 'plume' , 'interaction', '{"description": "A diffuse, elongated or plume-shaped low-surface-brightness structure extending from a galaxy, generally associated with tidal interaction, stripping or merger activity"}'::JSON )

-- , ( 'polar ring' , 'peculiar', '{"description": "A ring-like structure of gas, stars, or both orbiting a galaxy in a plane approximately perpendicular to the main stellar disk"}'::JSON )
-- , ( 'polar disk' , 'peculiar', '{"description": "An extended disk of gas, stars, or both whose rotation plane is strongly inclined, typically close to perpendicular, to the main stellar disk"}'::JSON )
-- , ( 'off-center' , 'peculiar', '{"description": "A nucleus whose photometric center is significantly displaced from the center of the outer galaxy body or disk"}'::JSON )
-- , ( 'lopsided' , 'peculiar', '{"description": "Asymmetric distribution of the galaxy light or structural components about its center, such that one side of the galaxy is significantly more extended or prominent than the other"}'::JSON )

-- , ( 'multiplicity' , ARRAY[0,4], 'environment',
-- '{
-- "description": "Abundance of galaxies brighter than main+5mag within R<=0.75*D25", 
-- "values": {
--   "0": "no other galaxy", 
--   "1": "one neighbour", 
--   "2": "two neighbours", 
--   "3": "three neighbours", 
--   "4": "four or more neighbours"
--   }
-- }'::JSON )

-- , ( 'arm rotation' , 'spiral pattern',
-- '{
-- "description": "Direction of spiral pattern rotation: clockwise (East of North negative) or counterclockwise (East of North positive)", 
-- "values": {
...
, value	SmallInt	NOT NULL	REFERENCES morphology.HubbleSequence (t)	ON UPDATE cascade ON DELETE restrict
, em_value	Real
, ep_value	Real
...
COMMENT ON COLUMN morphology.t.record_id	IS '{"description": "Record ID", "ucd": "meta.id"}';
COMMENT ON COLUMN morphology.t.value	IS '{"description": "de Vaucouleurs numeric type", "ucd": "src.morph.type"}' ;
COMMENT ON COLUMN morphology.t.em_value	IS '{"description": "Lower error", "ucd": "stat.error"}' ;
COMMENT ON COLUMN morphology.t.ep_value	IS '{"description": "Upper error", "ucd": "stat.error"}' ;
COMMENT ON COLUMN morphology.t.method	IS '{"description": "Morphology measurement method ID", "ucd": "meta.id"}';
...
, CHECK ( (em_value IS NULL and ep_value IS NULL) or (em_value IS NOT NULL and ep_value IS NOT NULL) )
, PRIMARY KEY (record_id, attribute, method)
) ;

COMMENT ON TABLE  morphology.features	IS 'Catalog of morphological features' ;
COMMENT ON COLUMN morphology.features.record_id	IS '{"description": "Record ID", "ucd": "meta.id"}';
COMMENT ON COLUMN morphology.features.attribute	IS 'Attribute ID' ;
COMMENT ON COLUMN morphology.features.value	IS 'Attribute value' ;
...
COMMENT ON COLUMN morphology.features.method	IS '{"description": "Morphology measurement method ID", "ucd": "meta.id"}';
...
COMMENT ON TABLE  morphology.extra	IS 'Catalog of extra types' ;
COMMENT ON COLUMN morphology.extra.record_id	IS '{"description": "Record ID", "ucd": "meta.id"}';
COMMENT ON COLUMN morphology.extra.type	IS '{"description": "Extra morphology type", "ucd": "src.morph.type"}' ;
...
COMMENT ON TABLE  morphology.spiral_winding	IS 'Catalog of spiral pattern winding' ;
COMMENT ON COLUMN morphology.spiral_winding.record_id	IS '{"description": "Record ID", "ucd": "meta.id"}';
COMMENT ON COLUMN morphology.spiral_winding.winding	IS '{
"description": "Apparent winding direction of the spiral pattern on the sky", 
"values": {
...
COMMENT ON COLUMN morphology.spiral_winding.em_winding	IS 'Lower error of the winding direction' ;
COMMENT ON COLUMN morphology.spiral_winding.ep_winding	IS 'Upper error of the winding direction' ;
COMMENT ON COLUMN morphology.spiral_winding.method	IS '{"description": "Morphology measurement method ID", "ucd": "meta.id"}';
