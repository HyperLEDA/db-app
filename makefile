...
default-rules:
	PGPASSWORD=password psql -h localhost -p 6432 --dbname hyperleda -U hyperleda -c "\copy layer0.homogenization_rules (catalog,parameter,key,filters,priority,enrichment) FROM 'tests/assets/default_rules.csv' WITH ( FORMAT csv, HEADER true, QUOTE '\"', DELIMITER ',');"
