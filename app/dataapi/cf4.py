...
def to_grid_index(coord, delta=1000 / 128, n=128, length=1000):
    """Преобразование координаты в индекс сетки"""
    coord = coord + length / 2
    return np.clip(np.floor(coord / delta).astype(int), 0, n - 1)
...
def get_data_from_skycoord_redshift(skycoord, redshift):
    ...
    sgc = SkyCoord(sgl=sgl, sgb=sgb, distance=distance_mpc * u.Mpc, frame="supergalactic")
