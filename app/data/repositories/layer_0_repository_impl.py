...
class Layer0RepositoryImpl(Layer0Repository):
    ...
    def create_instances(self, instances: list[Layer0Model]):
        with self._data_repository.with_tx():
            for instance in instances:
                ...
                table_resp = self._data_repository.create_table(creation)
    ...
    def fetch_data(self, param: Layer0QueryParam) -> list[Layer0Model]:
        with self._data_repository.with_tx():
            ...
            to_domain = []
            for table_id in ids:
                meta = self._data_repository.fetch_metadata(table_id)
                raw = self._data_repository.fetch_raw_data(table_id)
