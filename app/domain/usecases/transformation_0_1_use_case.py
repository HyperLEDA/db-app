...
class TransformationO1UseCase:
    ...
    def __init__(
        ...
        simultaneous_data_provider: Callable[[list[model.Layer0OldObject]], CrossIdSimultaneousDataProvider],
    ):
        """
    ...
    async def invoke(
        ...
        for coordinate, name in zip(coordinates, names, strict=False):
            if isinstance(coordinate, BaseException):
                ...
                identification_params.append(model.Layer0OldObject(None, None, coordinate))
            else:
                primary_name, all_names = name
                identification_params.append(model.Layer0OldObject(all_names, primary_name, coordinate))
