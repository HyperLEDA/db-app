...
RULES: list[NameRule] = [
    ...
    NameRule(
        ...
        pattern=re.compile(
            r"^(\[[a-z]{1,3}(?:[5-9]\d|20\d{2})\]|[a-z0-9]{2,6}?)\s*([JB]?)\s*(\d{7}[+-]\d{6,7})$", re.IGNORECASE
        ),
        replacement="",
        replacer=lambda m: f"{m.group(1).upper()} {m.group(2).upper()}{m.group(3)}",
    ),
    ...
    NameRule(
        name="CAT HHMMm+DDMM",
        pattern=re.compile(
            r"^(\[[a-z]{1,3}(?:[5-9]\d|20\d{2})\]|[a-z0-9]{2,6}?)\s*([JB]?)\s*(\d{5}[+-]\d{4,5})$", re.IGNORECASE
        ),
        replacement="",
        replacer=lambda m: f"{m.group(1).upper()} {m.group(2).upper()}{m.group(3)}",
    ),
...
def match_designation(name: str) -> tuple[str, str] | None:
...
def normalize_designation(name: str) -> str:
    match_result = match_designation(name)
